...
import { DATA_CATEGORY_PRODUCT_HAS_CHILREN, DATA_POSTS, DATA_PRODUCTS, getProductThumbnailRandom } from "@/domain/data-demo";
import { CategoryType, OrderStatusEnums, PostType, ProductType, StatusOrderType, StatusPostEnums, StatusPostType, TableDataType } from "@/domain/type";
...
type TablePostActionsProps = {
  classname?: string
}

export const TablePostActions = (props: TablePostActionsProps) => {
  const {
          isShow,
          changeShow,
          refPopup, isLeft
  ...
  return <div className={`${props?.classname ?? ''}`}>
    <div className={'flex gap-[14px] items-center justify-between flex-wrap'}>
      <h4 className={'text-size-2 mb-[0] line-clamp-1'}>Posts</h4>
        ...
        <div className={'inline-flex gap-[6px] flex-1 sm:flex-none'}>
          <Select<StatusPostType | 'delete'>
            ...
            options={[
              ...
              {
                value: 'delete',
                label: <span className={'text-alert'}>Xoá bài viết</span>
              },
            ]}
          />
          <Button
...
export const TablePost = (props: TablePostProps) => {
  const dataSources: TableDataType<PostType>[] = coverEntityToColumnType(DATA_POSTS).sort((a, b) => dayjs(b.createdat).unix() - dayjs(a.createdat).unix())

  const [columns, setColumns] = React.useState<TableColumnsType<TableDataType<PostType>>>([
      ...
      render: (_, item, index) => {
        return <div className={'flex gap-[10px] z-0 relative min-w-[240px]'}>
          ...
          <SystemLink
            ...
            className={'flex-1 block font-[5010] mb-0 line-clamp-2 text-title transition-colors hover:text-primary'}
